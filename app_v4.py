...
fichiers_requis = [TGVMAX_FILE, "etablissements-labellises-qualite-tourisme.csv", "coord_villes.csv"]
...
def load_data():
    tgvmax = pd.read_csv(TGVMAX_FILE, sep=";", dtype=str, encoding="utf-8", compression="gzip")
...
with col_info:
    ...
    st.markdown(
        f"""
        Le calculateur ci-dessous utilise uniquement le fichier local **{TGVMAX_FILE}**
...
st.caption(
    f"Le calculateur utilise uniquement le fichier local {TGVMAX_FILE} pour trouver, "
